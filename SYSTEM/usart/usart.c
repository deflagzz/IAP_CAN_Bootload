...
void usart_init(u32 bound)
{
	...
	USART_ITConfig(USART1,USART_IT_IDLE,ENABLE);       //只开启串口空闲中断
...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1,USART_IT_IDLE) != RESET)
	{
		...
		USART_RX_BUF[usart_recv_len] = '\0';  //划定要显示的长度
		DMA_SetCurrDataCounter(DMA1_Channel5,65535);//DMA通道的DMA缓存的大小
		DMA_Cmd(DMA1_Channel5, ENABLE);  //使能USART1 RX DMA1 所指示的通道 
	}
	
		

	//	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //串口接收数据，超时法，10ms*5
//	{
//		
